...
The application is a client-side visual dungeon generator whose domain and application layers run synchronously on the browser main thread. Strict supported-size and work limits protect responsiveness. One initial deterministic generator implements a replaceable strategy contract. A separate validator decides whether candidates satisfy structural and configured playability rules. A synchronous application service owns normalization, seeded randomness, bounded retries, validation, and result assembly. A separate play-session evaluator owns cardinal movement, completion, reset, and restored-session validation without mutating dungeon topology.

The web controller adapts this core to browser interactions. It coordinates an in-memory state store, a committed Canvas renderer, an accessible DOM view, and browser-local storage limited to exactly the most recent settings, accepted result, and compatible play-session state. Expected failures travel through typed result values; unexpected faults terminate at one browser error boundary.
...
| Interaction state | In-memory state plus one local latest record | Restores exactly one result, settings, and compatible play-session state; no catalog, history, account, cloud, or sync capability |
| Play-session boundary | Separate evaluator from generation acceptance | Movement cannot mutate dungeon topology, settings, or validation results |

## Component Architecture

| Layer | Components | Responsibility |
|---|---|---|
| Domain core | C-01 Domain Model, C-02 Seeded Random Source, C-03 Generation Strategy, C-04 Dungeon Validator, C-05 Settings Processor, C-13 Play Session Evaluator | Represent, generate, validate, and evaluate play movement without browser dependencies |
| Application layer | C-06 Generation Service, C-07 Application State Store, C-08 Version Metadata Provider | Orchestrate accepted outcomes, play-session state transitions, retry boundaries, and version metadata |
| Browser adapters | C-09 Web Controller, C-10 Canvas Renderer, C-11 Latest Result Storage, C-12 Accessible Web View | Adapt user events, Canvas, local storage, DOM status, metadata, accessibility, and play controls |
...
| `PlaySessionEvaluator` | C-13 | `attemptMove(dungeon, session, direction): MoveOutcome` |
| `GenerateDungeonUseCase` | C-06 | `execute(rawSettings): GenerationOutcome` |
| `ApplicationStateStore` | C-07 | Apply settings, busy, success, failure, play, reset, and restoration transitions |
| `CanvasDungeonRenderer` | C-10 | `render(result, session, options): RenderOutcome` |
| `LatestResultRepository` | C-11 | Load, save, clear, serialize, and deserialize one latest record |
| `DungeonView` | C-12 | Bind actions and render controls, status, diagnostics, metadata, play controls, and accessible summary |
...
6. A successful result atomically replaces application state, initializes play at the entrance, and is offered to the one-record local repository.
7. An expected failure updates diagnostics while preserving the previous valid result and play session.
8. View and Canvas adapters consume a completed immutable state snapshot.

### Play

1. The controller accepts cardinal movement commands when the play surface owns the intended interaction context.
2. The play-session evaluator accepts or rejects one cardinal move without mutating the dungeon.
3. Accepted moves update play-session state and trigger best-effort local persistence.
4. Reaching the exit updates completion state and presents a completion message through the view.
5. An explicit reset returns the character to the entrance and clears completion.
...
3. The play-session evaluator validates restored play state against the accepted dungeon.
4. A compatible record restores settings, one accepted result, and valid play-session state atomically.
5. Absent data leaves defaults unchanged.
6. Malformed, incompatible, or invalid play-session data is safely discarded and cannot block startup.
...
- C-13 alone owns cardinal movement and play-session validity rules.
...
| FR-10 | US-09, US-12 | C-07, C-09, C-11, C-13 |
| FR-11 | US-11 | C-01, C-07, C-09, C-10, C-12, C-13 |
| FR-12 | US-12 | C-07, C-09, C-11, C-12, C-13 |
| NFR-01 | US-01, US-02, US-06 through US-12 | C-07, C-09, C-10, C-12 |
| NFR-02 | US-01, US-02, US-06 through US-12 | C-09, C-10, C-12 |
| NFR-03 | US-03, US-07, US-10 | C-05, C-06, C-09, C-10 |
| NFR-04 | US-02 through US-06, US-11, US-12 | C-01 through C-09, C-13 |
| NFR-05 | US-03, US-10 | Layer boundaries and all explicit component contracts |
| NFR-06 | US-04, US-05, US-10, US-11 | C-02, C-03, C-04, C-06, C-11, C-13 test seams |
...
- Play-session movement is deterministic given dungeon topology and command sequence; it does not alter generation inputs.
...
- Cardinal movement evaluation remains lightweight relative to generation and rendering.
...
- C-12 owns keyboard controls, programmatic labeling, status announcements, focus, diagnostics, completion messaging, reset controls, and essential non-Canvas summaries.
- C-10 owns non-color visual distinctions, character rendering, and Canvas inspection controls.
- Movement commands apply only while the play surface owns the intended interaction context.
...
- The previous accepted result and compatible play session survive failed generation.
...
- Invalid restored play state cannot enter application state.

### Testability

- Domain and application contracts accept immutable values and injectable dependencies.
- Generation, validation, and play-session evaluation are browser-independent.
- C-11 provides an explicit serialization round-trip seam including play-session state.
- C-13 provides movement and completion invariant seams.
...
The design includes rooms, corridors, entrance, exit, settings, validation, visualization, metadata, regeneration, recovery, keyboard-playable entrance-to-exit navigation, and exactly one latest browser-local record with compatible play-session state. It excludes loot, encounters, enemies, combat, inventory, health, scoring, timers, multiplayer, collaboration, authentication, accounts, a saved catalog, multiple-result history, cloud storage, synchronization, and a public integration API.
...
- Character icon or sprite selection and Canvas movement rendering details
- Keyboard-event ownership, focus behavior, and reset affordance specifics
